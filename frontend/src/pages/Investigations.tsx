...
const sevColor: Record<string, string> = { Critical: "text-red-400 border-red-400/30", High: "text-orange-400 border-orange-400/30", Medium: "text-yellow-400 border-yellow-400/30", Low: "text-green-400 border-green-400/30" };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {incidents.map((inc) => (
          <div key={inc.id} onClick={() => navigate(`/investigations/${inc.id}`)} className="cursor-pointer">
            <Card className="transition-colors hover:border-primary/50">
              <CardHeader className="flex-row items-start justify-between space-y-0">
                <div><CardTitle className="text-base">{inc.title}</CardTitle><p className="text-xs text-muted-foreground mt-1">{inc.date}</p></div>
                <Badge variant="outline" className={sevColor[inc.severity]}>{inc.severity}</Badge>
              </CardHeader>
              <CardContent className="flex gap-2">
                <Badge variant="secondary">{inc.status}</Badge>
                {inc.techniques.map((t) => (<Badge key={t} variant="outline">{t}</Badge>))}
              </CardContent>
            </Card>
          </div>
        ))}
