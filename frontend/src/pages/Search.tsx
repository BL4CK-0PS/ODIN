...
export function SearchPage() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div><h1 className="text-3xl font-bold">Search</h1><p className="text-muted-foreground mt-1">Find similar investigations across institutional memory</p></div>
      <SearchBar value={query} onChange={setQuery} />
      <div className="space-y-3">{mock.map((r, i) => (<SimilarityCard key={i} title={r.title} score={r.score} reasons={r.reasons} />))}</div>
    </div>
  );
}
